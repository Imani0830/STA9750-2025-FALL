...
2. [Task 2: Data Analysing](#data-analysing)
...
To identify cities where federal YIMBY incentives would have the greatest impact, we recommend two straightforward metrics:
...
Housing Growth – This measures the pace of new housing development, both relative to the existing population and relative to population growth over a five-year window. Cities with low housing growth relative to population demand are constrained by zoning, permitting, or construction bottlenecks.

By combining these metrics, the government can classify cities as “high-YIMBY” candidates areas with high rent pressures but, strong potential for housing expansion. Federal funding can be targeted to cities meeting these criteria, enabling local governments to implement more permissive zoning, streamline permitting, and push developers to build housing that meets local needs.


### Graphs

Average houshold size over time (all CBSAs): 
